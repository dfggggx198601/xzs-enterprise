...
import { dashboardApi, userApi, agentApi } from '../../api';
...
export default function DashboardScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [dashRes, msgRes, agentRes] = await Promise.all([
        dashboardApi.index(),
        userApi.getMessageCount(),
        agentApi.list(),
      ]);
      if (dashRes.code === 1) {
        setDashData(dashRes.response);
      }

      if (msgRes.code === 1) {
        setUnreadCount(msgRes.response || 0);
      }

      if (agentRes.code === 1) {
        setAgents(agentRes.response || []);
      } else {
        setAgents([]);
      }
    } catch {
        ...
        {agents.length > 0 && (
          <View style={styles.section}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              <Text style={styles.sectionTitle}>我的微应用</Text>
              <TouchableOpacity onPress={() => navigation.navigate('AiChat')}>
                <Text style={{ color: '#2563eb', fontSize: 14 }}>开发新应用 &gt;</Text>
              </TouchableOpacity>
            </View>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginTop: 8 }}>
              {agents.map((agent: any) => (
                <TouchableOpacity
                  key={agent.id}
                  style={[styles.agentCard, { width: '48%', marginRight: 0, marginBottom: 12, marginTop: 0 }]}
                  onPress={() => navigation.navigate('AiChat', { id: agent.id, name: agent.name, desc: agent.description })}
                  activeOpacity={0.8}
                >
                  <LinearGradient colors={['#f8fafc', '#f1f5f9']} style={styles.agentCardInner}>
