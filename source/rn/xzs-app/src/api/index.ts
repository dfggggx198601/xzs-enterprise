...
class ApiClient {
  ...
  constructor() {
    this.client = axios.create({
      baseURL: BASE_URL,
      timeout: 120000, // Increased to 120s for AI APIs
      headers: {
...
export const agentApi = {
  create: (data: any) => apiClient.post('/api/student/agent/create', data),
  list: () => apiClient.post('/api/student/agent/list'),
  conversations: () => apiClient.post('/api/student/agent/conversations'),
  chat: (data: any) => apiClient.post('/api/student/agent/chat', data),
  history: (conversationId: number) => apiClient.post(`/api/student/agent/history/${conversationId}`),
};
