...
public class QuestionController extends BaseApiController {
    ...
    @RequestMapping(value = "/importFromFileLibrary", method = RequestMethod.POST)
    public RestResponse importFromFileLibrary(@RequestParam("file") MultipartFile file,
                                              @RequestParam("subjectId") Integer subjectId,
                                              @RequestParam("tag") String tag,
                                              @RequestParam(value = "regulationSource", required = false) String regulationSource) {
        if (file.isEmpty()) {
            return RestResponse.fail(2, "文件不能为空");
        }
        if (subjectId == null || StringUtils.isBlank(tag)) {
        ...
        String fileName = file.getOriginalFilename() == null ? "file-library" : file.getOriginalFilename();
        try {
            int count = questionService.importQuestionsFromFileLibrary(file.getInputStream(), fileName, subjectId, tag,
                    getCurrentUser().getId(), regulationSource);
            return RestResponse.ok(count);
        } catch (Exception e) {
            return RestResponse.fail(2, "文件库导题失败: " + e.getMessage());
        }
    }
}
