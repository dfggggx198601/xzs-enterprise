<template>
  <div class="app-container">
    <div style="margin-bottom: 15px;">
      ...
      <el-button type="warning" size="small" @click="handleLibraryImport">文件库导题</el-button>
    ...
    <el-dialog title="文件库导题" :visible.sync="libraryDialogVisible" width="520px">
      <el-form :model="libraryForm" label-width="110px">
        <el-form-item label="题库标签" required>
          <el-input v-model="libraryForm.tag" placeholder="请输入题库名称（如：反洗钱制度题库）"></el-input>
        </el-form-item>
        <el-form-item label="部门" required>
          <el-select v-model="libraryForm.level" placeholder="请选择部门" @change="libraryLevelChange" clearable>
            <el-option v-for="item in levelEnum" :key="item.key" :value="item.key" :label="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="业务范围" required>
          <el-select v-model="libraryForm.subjectId" placeholder="请选择业务范围" clearable>
            <el-option v-for="item in librarySubjectFilter" :key="item.id" :value="item.id" :label="item.name"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="制度来源">
          <el-input v-model="libraryForm.regulationSource" placeholder="例如：反洗钱法（2024版）"></el-input>
        </el-form-item>
        <el-form-item label="文本文件" required>
          <el-upload
            ref="libraryUpload"
            action=""
            :auto-upload="false"
            :limit="1"
            :on-change="handleLibraryFileChange"
            :on-remove="handleLibraryFileRemove"
            accept=".txt,.md,.csv">
            <el-button size="small" type="primary">选择文件</el-button>
            <div slot="tip" class="el-upload__tip">支持 .txt/.md/.csv（按行生成简答题）</div>
          </el-upload>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="libraryDialogVisible = false">取 消</el-button>
        <el-button type="primary" :loading="libraryLoading" @click="submitLibraryImport">开始导题</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.initSubject(function () {
      ...
      _this.librarySubjectFilter = _this.subjects
    })
  },
  methods: {
    fetchData () {
    ...
    handleLibraryImport () {
      this.libraryForm.tag = ''
      this.libraryForm.level = null
      this.libraryForm.subjectId = null
      this.libraryForm.regulationSource = ''
      this.libraryFile = null
      this.libraryDialogVisible = true
      this.$nextTick(() => {
        if (this.$refs.libraryUpload) {
          this.$refs.libraryUpload.clearFiles()
        }
      })
    },
    libraryLevelChange () {
      this.libraryForm.subjectId = null
      this.librarySubjectFilter = this.subjects.filter(data => data.level === this.libraryForm.level)
    },
    handleLibraryFileChange (file) {
      this.libraryFile = file.raw
    },
    handleLibraryFileRemove () {
      this.libraryFile = null
    },
    importLevelChange () {
      this.importForm.subjectId = null
    ...
    submitLibraryImport () {
      if (!this.libraryForm.tag) {
        this.$message.error('请输入题库标签')
        return
      }
      if (!this.libraryForm.subjectId) {
        this.$message.error('请选择业务范围')
        return
      }
      if (!this.libraryFile) {
        this.$message.error('请选择文本文件')
        return
      }
      this.libraryLoading = true
      questionApi.importFromFileLibrary(
        this.libraryFile,
        this.libraryForm.subjectId,
        this.libraryForm.tag,
        this.libraryForm.regulationSource
      ).then(response => {
        if (response.code === 1) {
          this.$message.success('导题成功，共生成 ' + response.response + ' 道题目')
        ...
        this.$message.error('导题失败: ' + (e.message || e))
      }).finally(() => {
