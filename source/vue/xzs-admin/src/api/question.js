...
export default {
  ...
  importFromFileLibrary: (file, subjectId, tag, regulationSource) => {
    const formData = new FormData()
    formData.append('file', file)
    formData.append('subjectId', subjectId)
    formData.append('tag', tag)
    if (regulationSource) {
      formData.append('regulationSource', regulationSource)
    }
    return axios.request({
      baseURL: process.env.VUE_APP_URL,
      url: '/api/admin/question/importFromFileLibrary',
      method: 'post',
      withCredentials: true,
      timeout: 120000,
      data: formData,
