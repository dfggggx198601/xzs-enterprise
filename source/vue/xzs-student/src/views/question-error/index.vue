    ...
    <el-row :gutter="50">
      <el-col :span="14">
        <el-table v-loading="listLoading" :data="tableData" fit highlight-current-row style="width: 100%" @row-click="itemSelect">
          ...
          <el-table-column prop="subjectName" label="业务范围"  width="50" />
          <el-table-column prop="regulationSource" label="制度来源" show-overflow-tooltip width="180" />
      ...
      <el-col  :span="10" >
        <el-card  class="record-answer-info">
          <el-form>
            <el-form-item label="制度来源" v-if="selectItem.regulationSource">
              <el-tag type="info">{{ selectItem.regulationSource }}</el-tag>
            </el-form-item>
            <el-form-item>
  ...
  methods: {
    ...
    qAnswerShow (id) {
      ...
      questionAnswerApi.select(id).then(re => {
        ...
        _this.selectItem.regulationSource = response.questionVM.regulationSource
